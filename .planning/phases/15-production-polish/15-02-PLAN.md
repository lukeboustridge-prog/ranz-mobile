...
<task type="auto">
  <name>Task 3: Update api-client to use config</name>
  <files>src/services/api-client.ts</files>
  <action>
Update `src/services/api-client.ts` to use the new environment config:
...
This centralizes API URL configuration and ensures it matches the environment.
  ...
  <verify>
```bash
cd /c/Users/LukeBoustridge/Projects/RANZ/RANZ\ Roofing\ Report/ranz-mobile && npx tsc --noEmit src/services/api-client.ts
```
  </verify>
  <done>
api-client.ts imports config from environment.ts and uses config.apiUrl for the base URL. Logging respects config.enableLogs.
  </done>
</task>
...
4. api-client.ts uses config.apiUrl
